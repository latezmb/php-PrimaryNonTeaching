...
if ($_SESSION['name']) {
    ...
    if (isset($_POST['year']) && isset($_POST['bigType'])){
        ...
        if ($bigType==""){
            $statement = "select * from work where year=? and status='待审'";
            $statementObject = $pdo->prepare($statement);
            $boo = $statementObject->execute(array($year));
        }else{
            $statement = "select * from work where year=? and bigType=? and status='待审'";
            $statementObject = $pdo->prepare($statement);
...
<div id="page-wrapper">
	...
	<div id="page-inner">
		<div class="row">
			<div class="col-md-12">
				 <form action="adminjobaudit.php?typename=adminjobaudit" method="post">
               &nbsp;&nbsp;&nbsp;&nbsp;
						年度：<select  name="year">
        ...
        foreach ($data as $information) {
            ...
            ?>
										...
										<td><a href="shenhefeiwork.php?typename=<?= $information['id']?>" class='btn btn-primary btn-xs'><i
												class='fa fa-edit'>&nbsp;审核</i></a>&nbsp;&nbsp; <a
											href="deleteinformation.php?adminjobauditid=<?= $information['id']?>"
