...
if ($_SESSION['name']) {
    ...
    if (isset($_POST['year'])) {
        $year = $_POST['year'];
        $statement = "select B.name,A.year,B.username,A.classHour,C.zong from user B left join teachhour A  on A.teacherId=B.username LEFT join (select year,teacherId,SUM(classHour) as zong from work where status='确认' group by teacherId,year) C on B.username=C.teacherId and A.year=C.year where A.teacherId=? and A.status='确认'and A.year=?";
        ...
        $statement = "select B.name,A.year,B.username,A.classHour,C.zong from user B left join teachhour A  on A.teacherId=B.username LEFT join (select year,teacherId,SUM(classHour) as zong from work where status='确认' group by teacherId,year) C on B.username=C.teacherId and A.year=C.year where A.teacherId=? and A.status='确认'";
        $statementObject = $pdo->prepare($statement);
        $boo = $statementObject->execute(array(
            $teacherId
        ));
...
<div id="page-wrapper">
	...
	<div id="page-inner">
		<div class="row">
			<div class="col-md-12">
				<form action="mytongjiwork.php?typename=mytongjiwork" method="post">
					&nbsp;&nbsp;&nbsp;&nbsp; 年度：<select name="year">
