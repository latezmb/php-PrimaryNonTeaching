    ...
    if (isset($_GET['typename'])) {
        ...
        if (isset($_POST['year'])) {
            ...
            if ($boo) {
                $row = $statementObject->rowCount();
                if ($row > 0) {
                    echo "<script language=\"JavaScript\">alert(\"审核成功！\");location.href='adminwoekshenhe.php?typename=adminwoekshenhe';</script>";
                    exit();
                } else {
                    echo "<script language=\"JavaScript\">alert(\"审核失败！\");location.href='adminwoekshenhe.php?typename=adminwoekshenhe';</script>";
                    exit();
                }
            } else {
			...
			<div class="col-md-8 col-md-offset-0">
				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<strong>审核授课工作信息</strong>
					</div>
					<div class="panel-body">
						<form role="form" id="workform" action="shenheworks.php"
							method='post'>
							...
							<div class="form-group input-group" id="amountdiv">
								...
								</span> <input type="text" required='required'
									class="form-control" id="amountClass" name="recorder"
									placeholder="记录人" value=<?= $data['recorder']?> />
							</div>
							...
							<input type='submit' name="save" class="btn btn-primary"
								...
								href="adminwoekshenhe.php?typename=adminwoekshenhe">放弃</a>
						</form>
					</div>
				</div>
			</div>
